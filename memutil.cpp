...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < op_cnt; i++)
	{
		...
		case OP_MAPREAD:
		{
			...
			size_t len;
			char *mem;
			mem = (char *)mmap(NULL, MYDEV_LEN, PROT_READ, mmap_flags, dev_fd, off);
			if (mem == MAP_FAILED)
			{
				perror("mmap read error"); //Prints reason for error along with mmap read error message
				ret = -1;
				break;
			}
			if (msg != NULL) //If no message was passed, there is nothing to read and compare to.
			{
				do // A do while loop is used becuase the msg has to be read at least once.
				{
					if (msg_len < MYDEV_LEN - off) //If the message is shorter in length that the remaining space in the device file, we can expect the entire message to be there in the file.
						len = msg_len;
					else // Otherwise, we expect to only find the first (MYDEVLEN - off) characters of the message string in the file, as the file terminates after that.
						len = MYDEV_LEN - off;

					for (int i = 0; i < len; i++) // Compares the characters in the msg and the characters read from the memory one by one
					{
						if (msg[i] != *(mem + off + i)) //Character Mismatch
						{
							cerr << "Read error";
							ret = -1;
						}
		...
		case OP_MAPWRITE:
		{
			...
			size_t len;
			char *mem;
			mem = (char *)mmap(NULL, MYDEV_LEN, PROT_WRITE, mmap_flags, dev_fd, off);
			if (mem == MAP_FAILED)
			{
				perror("mmap write error"); //Prints reason for error along with mmap write error message
				ret = -1;
				break;
			}
			do
			{
				...
				for (int i = 0; i < len; i++)  //Writes the message string into memory, one character at a time.
					*(mem + off + i) = msg[i];
